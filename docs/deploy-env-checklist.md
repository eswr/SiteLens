...
- `WEB_ORIGIN` must match allowed frontend origin(s). Use a comma-separated
  ...
  SiteLens Vercel previews matching
  `https://sitelens-*-easwarendra-kokas-projects.vercel.app` are allowed by
  pattern. Wrong CORS = browser API calls fail with “Failed to fetch”. Prefer
  keeping local Vite on `:5173` (`npm run dev:web:prod-api` uses `--strictPort`).
