...
function activitySection(
  count: number,
  byStatus: DevelopmentActivitySummary[],
): PlanningSummarySection {
  if (count === 0) {
    return {
...
function buildNextChecks(result: SpatialAnalysisResult): string[] {
  ...
  if (
    ...
    result.averageDevelopmentScore >= 70
  ) {
    checks.push(
 ...
 *
 * Pure function, **no external API/LLM calls**: the same `analysisResult`
 * always yields the same text (aside from the `generatedAt` timestamp).
 */
export function generatePlanningSummary(
  input: PlanningSummaryRequest,
): PlanningSummary {
  ...
  const siteContext =
    result.parcelCount === 0
      ? `This ~${result.areaHectares} ha area does not intersect any mapped parcels${
          topZone
            ? `, though it sits within ${topZone.zoneCode} · ${topZone.zoneName} zoning.`
            : '.'
        }`
